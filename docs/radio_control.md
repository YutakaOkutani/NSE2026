...
この機能は、ドローン格納中の無線干渉を避けるために、本番 systemd 起動時だけ Raspberry Pi Zero 2 W の Wi-Fi を停止し、放出判定またはタイムアウトで復帰させるためのものです。

## 設計方針

- 通常の手動実行では Wi-Fi を切らない。
- `CANSAT_RADIO_CONTROL=mission` のときだけ有効にする。
...
`cansat.service` には `EnvironmentFile` を追加する。
...
- 環境変数が `mission` でない限り Wi-Fi 操作を呼ばない。
...
- `log/<machine>/<run_id>/` に CSV が残り、`RadioDisabled` / `RadioLastEvent` / `RadioRestoreDeadlineElapsedSec` が記録される。
...
- 通常手動実行で Wi-Fi が切れない。
- systemd + `CANSAT_RADIO_CONTROL=off` で Wi-Fi が切れない。
- systemd + `CANSAT_RADIO_CONTROL=mission` + 短い復帰期限で Wi-Fi が切れて戻る。
- 復帰後に SSH でログを読める。
