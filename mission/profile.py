...
def _read_marker_profile() -> MachineProfileResolution | None:
    try:
        if not _MARKER_PATH.exists():
            return None
        marker_value = _MARKER_PATH.read_text(encoding="utf-8").splitlines()[0]
    except (OSError, IndexError):
        return None
    profile_name = _require_known_profile(_normalize_profile_name(marker_value), f"marker:{_MARKER_PATH}")
    return MachineProfileResolution(name=profile_name, source=f"marker:{_MARKER_PATH}")
...
def resolve_machine_profile(profile_name: str | None = None) -> MachineProfileResolution:
    ...
    # 明示指定や machine.txt が未知値だった場合は上で ValueError にする。
